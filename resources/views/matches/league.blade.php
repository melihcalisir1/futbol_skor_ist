...
<div class="container mx-auto px-4 py-8">
    <h1 class="text-3xl font-bold mb-6">{{ strtoupper(str_replace('-', ' ', $league)) }} Maçları</h1>
            ...
            <div class="bg-white shadow-md rounded-lg p-4">
                <div class="flex justify-between items-center mb-4">
                    ...
                    @if($match['status'] === 'LIVE')
                        <span class="text-red-500 font-bold">{{ $match['score']['home'] }} - {{ $match['score']['away'] }}</span>
                    @elseif($match['status'] === 'FINISHED')
                        <span class="text-green-500 font-bold">{{ $match['score']['home'] }} - {{ $match['score']['away'] }}</span>
                    @else
                        <span class="text-gray-600">{{ $match['time'] }}</span>
                    @endif
                    <div class="flex items-center">
